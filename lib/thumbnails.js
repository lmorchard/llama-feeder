...
export async function backfillThumbnails(db, log, options = {}) {
  ...
  await Promise.all(
    ids.map((id) =>
      jobQueue.add(() => fetchThumbnailForFeedItem(id, db, log), {
        meta: id,
      })
    )
  );
...
export async function fetchThumbnailForFeedItem(id, db, log) {
  ...
  try {
    ...
    if (err.type === "AbortError") {
      ...
      log.error({ msg: "Failed to backfill thumbnail", id, message: err.message });
    }
  }
