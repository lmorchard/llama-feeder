...
import { generateEmbeddingsBatch, getEmbedContentForFeedItem } from "../llamafile.js";
...
async function commandBackfill(options) {
  ...
  for (let idx = 0; idx < feedItemIds.length; idx += batchsize) {
    ...
    const content = feedItems.map(getEmbedContentForFeedItem);
    const embeddings = await generateEmbeddingsBatch(content);
