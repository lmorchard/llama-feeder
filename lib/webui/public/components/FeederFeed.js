...
export default class FeederFeed extends LitElement {
  ...
  get limit() {
    return parseInt(this.parent.getAttribute("limit"), 100);
  }
  ...
  async loadFeed() {
    ...
    const url = `${this.apiUrl}/feeds/${feedId}?maxage=${this.maxage}&limit=${this.limit}`;
      ...
      <details open=${!!this.feed}>
        ...
        <section class="items">
          <ul>
            ${this.items &&
            this.items.map(
              ({ title, link, text, author, thumbnailUrl, date }) => html`
                <li>
                  <feeder-feed-item
                    title="${title}"
                    link="${link}"
